...
# https://eev.ee/blog/2011/04/24/gotcha-python-scoping-closures/)
    ...
    def getFilterBandwidth(self, filter) -> Optional[Tuple[float, float]]:
        ...
        if bandwidth == model.BAND_PASS_THROUGH:
            return None
        elif not isinstance(bandwidth, (tuple, list)) or len(bandwidth) != 2:
            logging.info("Invalid filter (%s) bandwidth: %s", filter.name, bandwidth)
            return None

        return bandwidth

    def updateOutWLFilterAndSpectrograph(self, comp_affected, filter, spectrograph):
        """
        ...
        logging.debug("Computed intersection of filter %s and spectrograph %s -> %s",
                      filter_bandwidth, spec_bandwidth, bandwidth)
    ...
    def observeFilter(self, filter, comp_affected):
        ...
        def updateOutWLRange(pos, fl=filter, comp_affected=comp_affected):
            self.updateOutWLFilterAndSpectrograph(comp_affected, fl,
                                                  self._det_to_spectrograph.get(comp_affected.name))
