...
class TrueForm(model.Emitter):
    def __init__(self, name, role, address,
        ...
        if channel not in {1, 2}:
            raise ValueError("Channel must be 1 or 2, got %s" % (channel,))
        ...
        for c, t in self._tracking.items():
            ...
            if c not in {1, 2}:
                raise ValueError("Tracking channel must be 1 or 2, got %s" % (c,))
            if t not in {"ON", "INV"}:
    ...
    def setFrequency(self, c: int, f: float):
        """
        c (1 or 2): the channel to set the frequency of.
        f: the frequency value in Hertz
        """
    ...
    def getFrequency(self, c: int) -> float:
        """
        c (1 or 2): the channel to get the frequency of.
        """
    ...
    def _update_settings(self):
        ...
        """
        f = self.getFrequency(self._channel)
        p = 1 / f
