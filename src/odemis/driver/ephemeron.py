...
class MightyEBIC(Detector):
    def __init__(self, name: str, role: str, channel: int, url: str, **kwargs):
        ...
        # Use a float for the depth, to indicate it returns a float.
    ...
    def _acquire_thread(self) -> None:
        ...
        try:
            ...
            if self._opc_client.controller_state != STATE_NAME_IDLE:
                logging.warning("Scan controller is not idle, will stop the current scan")
                self._opc_client.stop_scan()
                time.sleep(0.1)
            while self._opc_client.controller_state != STATE_NAME_IDLE:
                logging.warning("Scan controller is not idle, will wait for it to be idle")
                if self._acquisition_must_stop.wait(0.1):
                    self._opc_client.stop_scan()
                    return

            self._opc_client.start_trigger_scan(os, self._channel + 1, spp, delay, res[0], res[1], timeout=DEFAULT_TIMEOUT + 1)
            ...
            logging.debug("State after readying acq = %s", state)
            logging.debug("Acquisition started, expected scan time = %s s, so will wait for the data up to %s seconds",
                          scan_time, DEFAULT_TIMEOUT + scan_time)
            ...
            while self._opc_client.controller_state in (STATE_NAME_BUSY, STATE_NAME_TRIGGER):
                ...
                logging.debug("Waiting for the scan to finish, state is %s", self._opc_client.controller_state)  #DEBUG
                if self._acquisition_must_stop.wait(0.1):
                    self._opc_client.stop_scan()
                    return

            # It shouldn't happen, but for now, it often does
            while time.time() < earliest_start + scan_time:
                logging.warning("Waiting longer for the scan to really finish, state is %s", self._opc_client.controller_state)  # DEBUG
    ...
    def read_data(self, resolution: Tuple[int, int], channels: int, md: Dict[str, Any]) -> List[model.DataArray]:
        ...
        logging.debug("Will read data, with expecte shape %s", result_shape)
    ...
    def on_dwell_time_change(self, value: float) -> float:
        ...
        """
        # Find the closest SPP that matches the requested dwell time. It always returns a smaller or equal value,
        ...
        if not math.isclose(dt, value, abs_tol=10e-6):
...
class MightyEBICUaClient:
    ...
    def guess_samples_per_pixel_and_oversampling(self, req_dt: float, channels: int, delay: float) -> Tuple[float, int, int]:
        """
    ...
    async def _guess_samples_per_pixel_and_oversampling(self, req_dt: float, channels: int, delay: float) -> Tuple[float, int, int]:
        """
    ...
    async def _guess_samples_per_pixel(self, req_dt: float, oversampling: int, channels: int, delay: float) -> Tuple[float, int]:
        """
        ...
        if req_dt_us < dt_min:
            logging.warning(f"Requested dwell time {req_dt} µs is less than minimum dwell time {dt_min} µs.")
    ...
    async def _calculate_scan_time(self, dt: int, p_fast: int, p_slow: int) -> int:
        """
    ...
    async def _start_trigger_scan(self, oversampling: int, channels: int, spp: int, delay: float,
        ...
        # TODO: what the heck is the difference between oversampling and sampling per pixel?
        :param oversampling: The oversampling rate that is applied. Must be within {0, 2, 4, 8, 16, 32, 64}.
        TODO: more params
        :param channels: The number of channels used simultaneously.
        :param spp: The number of samples per pixel, which will be averaged in the result.
        :param delay: ??? in s? or ms?
        :param sim: If True, the scan will be simulated (by the server)
        :param timeout: The maximum time to wait for the trigger for the scan to start. If the trigger
    ...
    def get_scan_result(self) -> List[float]:
        """
        :return: the raw data of the latest scan result in the order it was acquired.
        """
    ...
    def get_scan_result_shape(self) -> Tuple[int, int, int]:
        """
        :return: p_fast, p_slow pixels of the scan result
        """
...
class MightyEBICSimulator(Server):
    ...
    async def calculate_dwell_time(self, parent, oversampling: int, channels: int, spp: int, delay: float) -> int:
        ...
        All constants are in nanoseconds. This method is a copy of the method Ephemeron uses in their code.
        :param parent:
        :param oversampling: The oversampling rate that is applied.
        :param channels: The number of channels used simultaneously.
        :param spp: The number of samples per pixel used, this value is determined by the requested dt.
        :param delay: Number of clock cycles, this is a variable delay that allows for the signal to
    ...
    async def request_scan_start(self, parent,
        ...
        :param timeout: maximum time to wait for the trigger
        """
