...
class SEMCCDAcquirerRectangle(SEMCCDAcquirer):
    ...
    def prepare_hardware(self, max_snapshot_duration: Optional[float] = None) -> None:
        ...
        if photon_counting:
            ...
            exp_time = integration_time / self._mdstream._integrationCounts.value   #FIXME, exp_time_va not already set?
            logging.debug("Integration time: %s s, exposure time: %s s", integration_time, exp_time)
            if max_snapshot_duration is None:
                max_snapshot_duration = exp_time
            else:
