...
class DelayConnector(model.HwComponent):
    """
    Wrapper component to connect a trigger component with the rest of the streak-cam components.
    """
    def __init__(self, name: str, role: str,
        ...
        * streak-unit: a component with a .timeRange, to be updated when the triggerDelay is changed
    ...
    def _on_trigger_period(self, period: float) -> None:
        ...
        """
        rate = 1 / period  # There is no reason it should be 0, so no need to check... and if it fails, it'll be logged
