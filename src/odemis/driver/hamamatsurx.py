...
class DelayGenerator(model.HwComponent):
    ...
    def __init__(self, name, role, parent, daemon=None, streak_unit=None, **kwargs):
        ...
        if "Lock Mode" in avail_params:
            locked = self.GetLockMode().lower() == "locked"
    ...
    def GetDelayByName(self, delay_name: str):
        ...
        if delay_name == "Delay Time":
            # TODO: what's the unit ? ps? ns? period?
            delay *= 1e-12  # On the C12270, the unit is different
    ...
    def _setDelayByName(self, delay_name: str, value: float):
        ...
        """
        if delay_name == "Delay Time":
            value /= 1e-12  # On the C12270, the unit is different
    ...
    def GetDelayRangeByName(self, delay_name: str) -> Tuple[float, float]:
        ...
        if delay_name == "Delay Time":
            max_time *= 1e-12  # On the C12270, the unit is different
    ...
    def _updateSettings(self) -> None:
        ...
        try:
            ...
            if hasattr(self, "phaseLock"):
                lock_mode = self.GetLockMode().lower()
                if lock_mode == "locked":
                    self.phaseLock._value = True
                elif lock_mode == "unlocked":
                    self.phaseLock._value = False
                    # TODO notify
                else:
                    logging.warning("Not sure what to do with lock mode %s", lock_mode)
