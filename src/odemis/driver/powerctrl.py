...
class PowerControlUnit(model.PowerSupplier):
    ...
    def __init__(self, name, role, port, pin_map=None, delay=None, init=None,
        ...
        init (dict str -> boolean): turn on/off the corresponding component upon
            initialization.
        ids (None or list[str]): EEPROM ids expected to be detected during initialization.
        All the IDs must be found. If not, another device is search for.
        excluded_ids (None or list[str]): EEPROM ids which must not be detected during initialization.
        If any ID listed is detected, another device is search for, until a device with none
        of these IDs is found.
    ...
    def _findDevice(self, ports: str, required_ids: List[str], excluded_ids: List[str]) -> Tuple[str, str]:
        ...
        for n in names:
            ...
            if required_ids or excluded_ids:
                ...
                if not all(eid in mem_ids for eid in required_ids):
